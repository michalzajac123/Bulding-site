<template>
	<div class="header-container top-0 bg-blue-800 z-100 text-white flex-col flex justify-center w-full">
		<div class="nav flex justify-between py-4 items-center w-full max-w-7xl mx-auto px-8 z-10">
			...
			<div class="nav-items space-x-12 text-lg hidden md1:flex">
				<div class="nav-item">
					<router-link to="/" class="text-white font-thin hover:text-[var(--primary-color)] transition-colors"
						>Strona Główna</router-link
					>
				</div>
				<div class="nav-item">
					<router-link to="/aboutus" class="text-white hover:text-[var(--primary-color)] transition-colors"
						>O nas</router-link
					>
				</div>
				<div class="nav-item">
					<router-link to="/" class="text-white hover:text-[var(--primary-color)] transition-colors"
		...
		<div class="absolute top-0 left-0 w-full h-full bg-blue-900 opacity-60 z-1"></div>

		<img :src="imageLink" class="z-0 absolute top-0 left-0 w-full h-full object-cover" alt="Home preview" />

		<!-- Treść na obrazie -->
		<div class="relative z-4 text-white max-w-7xl mx-auto px-8 w-full py-24">
			<div class="max-w-2xl mt-16">
				<h2 class="text-lg uppercase tracking-wider font-semibold mb-4">Firma budowlana</h2>
...
const imageLink = computed(() => {
	switch (route.path) {
		...
		case '/calculator':
			return new URL('../images/HomeCalculatePreview.jpeg', import.meta.url).href
		case '/contact':
